...
class Inference:
    ...
    def __init__(
        ...
        self._valid_set_path = os.path.join(self.bd_conf.model_path, 'valid_samples.csv')
    ...
    def inference_fn(self, in_dir, out_dir, valid_loader, device, min_t=None, num_t=None, noise_scale=1.0):
        
        for valid_feats, pdb_name in valid_loader:

            pdb_name = pdb_name[0]
            protein_pdb = open(os.path.join(in_dir, f'{pdb_name}.pdb'), 'r').readlines()
            ...
            sample_path = os.path.join(out_dir, f'{pdb_name}.pdb')
